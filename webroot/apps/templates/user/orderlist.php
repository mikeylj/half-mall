...
<!--    </div>-->
    ...
    <div class="inner">
        ...
        <p id = "mores" style="text-align: center; height:.4rem; line-height:.4rem; font-size:.14rem; "><a onclick="runs(<?php echo $id; ?>)">点击加载更多...</a></p>
    </div>
...
<script>
    ...
    function runs(userId){
        $("#mores").html("正在加载中...");
        $.post('/users/aj_orderlist',{page:sum,id:userId,srcFrom:'SRDB-TEST-001'}, function(response) {
            if (response.code == -1){
...
//                                +'<a style="z-index: 20;" class="buyList-getBtn"  href="/PswCheck/toPwLogin?srcFrom=SRDB-TEST-001">点击领取</a>'
                    ...
                    $("#mores").html('<a onclick="runs(<?php echo $id; ?>)">点击加载更多...</a>');
                } else{
                    $("#mores").hide();
                }
